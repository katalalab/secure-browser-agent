...
test('source audit inventories known local browser references and readiness signals', () => {
  ...
  const agentBrowserDir = path.join(homeDir, 'work/claude-skill-sources/repos/vercel-labs__agent-browser');
  const lightpandaDir = path.join(homeDir, 'work/nicolas-starred-repos/repos/lightpanda-io_browser');
